...
export function chequeFriendlyDeposits(totalTTC: number, nombreCheques: number): ChequeSuggestion[] {
  ...
  for (const pct of pourcentages) {
    ...
    if (montantParChequeEntier > 0 && acompteAjuste >= 0 && acompteAjuste <= totalTTC) {
  ...
  for (let montantCheque = 50; montantCheque <= 500; montantCheque += 50) {
    ...
    if (totalCheques < totalTTC) {
      ...
      if (pourcentage >= 0 && pourcentage <= 80) { // Limiter à 80% d'acompte max
        suggestions.push({
          deposit: acompte,
          nCheques: nombreCheques,
  ...
  const uniqueSuggestions = suggestions
    .filter((suggestion, index, self) => 
      index === self.findIndex(s => s.deposit === suggestion.deposit && s.perCheque === suggestion.perCheque)
    )
