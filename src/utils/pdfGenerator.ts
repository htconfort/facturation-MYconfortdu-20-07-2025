...
export const generatePDF = async (
  ...
  try {
    ...
    doc.text(`${invoice.subtotal.toFixed(2)} €`, 200, finalY, {
      align: 'right',
    });

    doc.text(`TVA (20%):`, 140, finalY + 8);
    doc.text(`${invoice.tax.toFixed(2)} €`, 200, finalY + 8, {
      align: 'right',
    });
    ...
    doc.text(`${invoice.total.toFixed(2)} €`, 200, finalY + 20, {
      align: 'right',
    });

    // Signature électronique
    if (options.signatureBase64) {
      doc.addImage(options.signatureBase64, 'PNG', 130, finalY + 35, 40, 20);
      doc.setFontSize(8);
      doc.setTextColor(0, 0, 0);
