 ...
 */
export const migrateProductsWithDeliveryStatus = (
  products: Product[]
): Product[] => {
  return products.map(product => {
    ...
    if (!product.statut_livraison) {
      return {
        ...product,
        statut_livraison: product.isPickupOnSite
          ? 'delivered'
...
export const validateDeliveryStatus = (
  ...
  if (product.isPickupOnSite && product.statut_livraison !== 'delivered') {
    errors.push(
      'Produit marqué comme pickup mais statut de livraison différent de "delivered"'
    );
  }
...
export const getDeliveryStatistics = (products: Product[]) => {
  ...
  return {
    ...
    completionPercent:
      total > 0 ? Math.round(((delivered + cancelled) / total) * 100) : 0,
 ...
 */
export const filterProductsByDeliveryStatus = (
  products: Product[],
  status: DeliveryStatus
): Product[] => {
  return products.filter(
    product => (product.statut_livraison || 'pending') === status
  );
};
