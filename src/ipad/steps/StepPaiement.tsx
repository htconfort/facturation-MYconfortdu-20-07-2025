...
export default function StepPaiement({ onNext, onPrev, onQuit }: StepProps) {
  ...
  const isValid = typeof paiement.method === 'string' && paiement.method.trim().length > 0;

  // Suggestions pour chèques sans virgule
  const chequeSuggestions =
    paiement.method === 'Chèque à venir' ? chequeFriendlyDeposits(totalTTC, nombreCheques) : [];
            ...
            <section className="bg-white rounded-2xl shadow-xl p-6">
              ...
              <div className="bg-gray-50 rounded-xl p-6">
                <div className="flex justify-between items-center text-lg mb-2">
                  <span>Total HT</span>
                  <span className="font-semibold">{formatEUR(totalTTC / (1 + TVA_RATE))}</span>
                </div>
                <div className="flex justify-between items-center text-lg mb-2">
                  <span>TVA (20%)</span>
                ...
                {paymentMethods.map((method) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className={`font-semibold ${selected ? 'text-black' : 'text-gray-800'}`}>
                        {method.label}
                      </div>
                      {method.priority && (
                        <div
              ...
              {depositAmount > 0 && (
                <div className="mb-6">
                    ...
                    onChange={(e) => {
                      ...
                      updatePaiement({
                        depositPaymentMethod:
                          value === '' ? undefined : (value as DepositPaymentMethod),
                      });
                    }}
                  ...
                  >
                    ...
                    <option value="Carte Bleue" className="text-blue-800 font-bold">
                      💳 Carte Bleue
                    </option>
                    <option value="Espèces" className="text-green-800 font-bold">
                      💵 Espèces
                    </option>
                    <option value="Chèque" className="text-purple-800 font-bold">
                      🧾 Chèque
                    </option>
                  </select>
                  {!paiement.depositPaymentMethod && (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="text-center p-4 bg-blue-100 rounded-xl">
                    ...
                    <div className="text-sm text-blue-500">
                      {totalTTC > 0 ? `${Math.round((depositAmount / totalTTC) * 100)}%` : '0%'} du
                      total
                    </div>
                  ...
                  <div className="bg-white rounded-xl p-6 border border-blue-200">
                    ...
                    <div className="mt-4 p-3 bg-green-50 rounded-lg border border-green-200">
                      <div className="text-sm text-green-700">
                        ✅ <strong>Avantage :</strong> aucun frais supplémentaire • flexibilité •
                        confort d&apos;achat
                      </div>
                    </div>
                  </div>
