...
export default function StepRecapNoScroll({ onNext, onPrev }: StepProps) {
  ...
  const totalTTC = useMemo(() => {
    return produits.reduce(
      (sum, p) =>
        sum +
        calculateProductTotal(p.qty, p.priceTTC, p.discount, p.discountType),
      0
    );
  }, [produits]);
  ...
  const isEmailSent = actionHistory.some(action =>
    action.includes('envoyée par email')
  );
  ...
  return (
    <div className='w-full h-full bg-myconfort-cream flex flex-col overflow-hidden relative'>
      ...
      {/* 🎯 Contenu principal - INTERFACE SIMPLIFIÉE AVEC SCROLL */}
        ...
        <div className='space-y-4'>
          ...
          {canProceed && (
            <div className='bg-myconfort-green/10 p-3 rounded-lg border border-myconfort-green/30 text-center'>
              ...
              <div className='text-sm font-bold text-myconfort-green'>
                Terminé !
              </div>
      ...
      {/* 🎯 NAVIGATION BUTTONS - Toujours visible */}
      <div className='px-6 py-4 border-t border-myconfort-dark/10 flex justify-between items-center bg-white'>
        <button
          onClick={onBack}
          ...
          {canProceed ? (
            <button
              ...
              className='px-6 py-3 bg-myconfort-green hover:bg-myconfort-green/90 
                       text-white rounded-lg font-bold transition-all shadow-lg
                       flex items-center gap-2 text-lg disabled:opacity-50
            ...
            <button
              ...
              className='px-6 py-3 bg-gray-300 text-gray-500 rounded-lg 
                       font-bold cursor-not-allowed flex items-center gap-2 text-lg'
            >
              <span>🔒</span>
              <span>Compléter d'abord</span>
            </button>
          )}
        </div>
      </div>
      ...
      <div className='flex-1 px-6 py-4 overflow-y-auto'>
        <div className='bg-white p-6 rounded-xl border border-gray-300 max-w-4xl mx-auto'>
          ...
          <div className='flex justify-between items-start mb-6'>
            <div>
              <h1 className='text-2xl font-bold text-myconfort-green'>
                MyConfort
              </h1>
          ...
          <div className='mb-6'>
            ...
            <div className='bg-blue-50 p-3 rounded'>
              ...
              {paiement.nombreFoisAlma && (
                <div>
                  <strong>Alma:</strong> {paiement.nombreFoisAlma} fois
                </div>
              )}
              {paiement.nombreChequesAVenir && (
                <div>
                  <strong>Chèques à venir:</strong> {paiement.nombreChequesAVenir} chèques
                  {paiement.montantChaqueChequesAVenir && (
                    <span className="text-myconfort-dark/80">
                      {' '}de {paiement.montantChaqueChequesAVenir}€ chacun
                    </span>
                  )}
                </div>
              )}
          ...
          <div className='mb-6'>
            ...
            <div className='bg-yellow-50 p-3 rounded'>
              <div>
                <strong>Mode:</strong> {livraison.deliveryMethod || 'À définir'}
              </div>
              <div>
                <strong>Adresse:</strong>{' '}
                {livraison.deliveryAddress || client.address}
              </div>
              {livraison.deliveryNotes && (
                <div>
                  <strong>Instructions:</strong> {livraison.deliveryNotes}
                </div>
              )}
            </div>
          </div>
...
function ProcessingPage({ onComplete }: { onComplete: () => void }) {
  ...
  return (
    <div className='w-full h-full bg-myconfort-cream flex flex-col overflow-hidden items-center justify-center'>
          ...
          {steps.map((stepText, i) => (
            <div
              ...
              className={`flex items-center justify-center gap-3 p-2 rounded-lg ${
                i < step
                  ? 'bg-myconfort-green/10 text-myconfort-green'
                  : i === step
                    ? 'bg-myconfort-blue/10 text-myconfort-blue'
      ...
      {/* 🎯 FOOTER RECAP - Toujours visible sur iPad */}
      <FloatingFooter>
