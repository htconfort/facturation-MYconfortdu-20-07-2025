...
export default function StepSignature({ onNext, onPrev }: StepProps) {
  ...
  return (
    <div className='w-full h-dvh p-6 flex flex-col gap-6 bg-myconfort-cream'>
      <h1 className='text-2xl font-semibold text-myconfort-dark'>
        Signature du client
      </h1>
      ...
      {signature?.dataUrl && (
        <div className='mt-2'>
          <p className='text-sm text-myconfort-dark/70'>
            Dernier enregistrement : {signature.timestamp}
          </p>
          <img
      ...
      <div className='mt-auto flex justify-between'>
        <button
          onClick={onPrev}
          className='px-8 py-4 bg-gray-200 rounded-xl font-bold'
        >
          Précédent
        </button>
        ...
        <button
          disabled
          className='px-8 py-4 bg-myconfort-green text-white rounded-xl font-bold opacity-50'
        >
          Suivant
        </button>
      </div>
