...
export default function StepPaymentNoScrollFixed({ onNext, onPrev }: StepProps) {
      ...
      <div className="flex-1 px-6 py-4 flex flex-col justify-center">
        ...
        <div className="mb-6">
          <label className="block text-sm font-medium text-myconfort-dark mb-2">
            Acompte (€) * - Minimum 10% = {(totalAmount * 0.10).toFixed(2)}€
          </label>
          ...
          <div className="flex gap-2">
            <input
              ...
              min={Math.round(totalAmount * 0.10)}
              max={totalAmount}
              ...
              placeholder={`Min: ${(totalAmount * 0.10).toFixed(2)}€`}
            />
            <button
              onClick={() => setAcompte(Math.round(totalAmount * 0.10))}
        ...
        <div className="grid grid-cols-3 gap-4">
          ...
          <button
            onClick={() => {
              ...
              updatePaiement({ method: 'Chèque au comptant', depositAmount: acompte });
            }}
            className={`p-4 rounded-xl border-2 transition-all ${
          ...
          >
            ...
            <div className="font-semibold text-blue-800">
              {selectedMethod === 'Chèque à venir' ? 'Chèques configurés' : 'Chèques à venir'}
            </div>
            <div className="text-sm text-blue-600">
              {selectedMethod === 'Chèque à venir' ? (
                <div className="space-y-1">
                  <div><strong>{paiement?.nombreChequesAVenir || 0} chèques</strong></div>
                  <div>
...
function ChequesDetailsPage({ 
  ...
  onBack: () => void; 
  onComplete: (data: { count: number; amount: number; notes: string; acompte: number }) => void; 
}) {
  ...
  const acompteMinimum = totalAmount * 0.10;
  ...
  const handleChequeCountChange = (count: number) => {
    ...
    setTimeout(() => {
      ...
      if (acomptePersonnalise && acomptePersonnalise >= totalAmount * 0.10) {
        ...
        const acompteMin = totalAmount * 0.10;
  ...
  return (
    <div className="w-full h-full bg-myconfort-cream flex flex-col overflow-hidden">
      ...
      <div className="px-6 py-4 border-b border-myconfort-dark/10">
        ...
        <p className="text-myconfort-dark/70 text-sm">
          Total: {totalAmount.toFixed(2)}€ • Règle: chèques ronds + acompte 10% minimum
        </p>
      </div>
      ...
      <div className="flex-1 px-6 py-6 flex flex-col justify-center space-y-6">
        ...
        <div>
          ...
          {isCalculating && (
            <div className="mb-3 p-2 bg-blue-100 border border-blue-300 rounded-lg text-center">
              <span className="text-blue-800 font-medium">🧮 Calcul en cours...</span>
            </div>
          )}
          ...
          <p className="text-sm text-blue-600 mt-2">
            💡 Cliquez sur un nombre pour calculer automatiquement l'acompte et revenir au mode de règlement
          </p>
        </div>
        ...
        <div className="bg-blue-50 p-4 rounded-xl border-2 border-blue-200">
          ...
          <div className="text-sm text-blue-700 space-y-1">
            <div><strong>Total TTC :</strong> {totalAmount.toFixed(2)}€</div>
            <div><strong>Nombre de chèques :</strong> {chequeCount}</div>
            <div><strong>Acompte minimum (10%) :</strong> {acompteMinimum.toFixed(2)}€</div>
            {(() => {
              ...
              const acompteCalculeBrut = totalAmount - (Math.round(totalAmount / chequeCount) * chequeCount);
              
              return (
                <>
                  <div><strong>Acompte calculé :</strong> {acompteCalculeBrut.toFixed(2)}€</div>
                  <div className={`font-bold ${acompteCalculeFinal >= acompteMinimum ? 'text-blue-800' : 'text-orange-600'}`}>
                    <strong>Acompte final :</strong> {acompteCalculeFinal.toFixed(2)}€
                    {acompteCalculeFinal > acompteCalculeBrut && ' (ajusté à 10% minimum)'}
                  </div>
                  <div><strong>Montant par chèque (rond) :</strong> {montantChequeFinal.toFixed(2)}€</div>
                  <div><strong>Total des chèques :</strong> {(montantChequeFinal * chequeCount).toFixed(2)}€</div>
                </>
              );
            })()}
          </div>
          {!isCalculating && (
            <div className="mt-2 p-2 bg-blue-100 rounded-lg">
              <p className="text-blue-800 text-sm font-medium">
                ✨ Cliquez sur un nombre ci-dessus pour confirmer et revenir automatiquement
              </p>
            </div>
          )}
        </div>
        ...
        <button
          onClick={isValid ? () => {
            ...
            onComplete({ 
              ...
              notes: notes || `${chequeCount} chèques de ${montantChequeFinal}€ + acompte ${acompteCalculeFinal.toFixed(2)}€` 
            });
          } : undefined}
          disabled={!isValid}
          className={`px-12 py-4 font-bold rounded-xl text-lg transition-all transform 
