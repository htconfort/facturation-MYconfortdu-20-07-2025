...
function WizardSurface({
  ...
  const StepComponent = useMemo(() => {
    ...
    switch (step) {
      ...
      case 'client':
        return <StepClient {...props} />;
      case 'produits':
        return <StepProduits {...props} />;
      case 'paiement':
        return <StepPaiement {...props} />;
      case 'livraison':
        return <StepLivraison {...props} />;
      case 'signature':
        return <StepSignature {...props} />;
      case 'recap':
  ...
  return (
    <div className='relative w-full h-full bg-gradient-to-br from-gray-50 to-white'>
      ...
      <div className='pt-16 pb-24 px-8 w-full h-full overflow-auto'>
        <div className='max-w-7xl mx-auto'>{StepComponent}</div>
      </div>
      ...
      <div className='absolute left-0 bottom-0 right-0 h-20 bg-white border-t-2 border-gray-200 flex items-center px-8 justify-between shadow-lg'>
        <div className='flex items-center gap-4'>
          ...
          {!isLastStep ? (
            ...
            <NavButton onClick={onQuit} label='✅ Terminer' variant='success' />
          )}
        </div>
...
function NavButton({
  ...
  const baseClasses =
    'px-6 py-3 rounded-xl font-semibold text-lg transition-all transform hover:scale-105 shadow-lg';
