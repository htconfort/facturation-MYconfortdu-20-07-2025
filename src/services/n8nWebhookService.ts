...
// Type pour les produits avec statut de livraison
interface ProductWithDeliveryStatus {
...
export class N8nWebhookService {
  ...
  static async sendInvoiceToN8n(
    ...
    message: string;
    response?: unknown;
    payload?: Record<string, unknown>; // Changé de ValidatedInvoicePayload à any pour accepter la structure N8N
  }> {
    try {
      ...
      console.log(
        '🔧 Création payload avec structure validée du commit e54c7f9...'
      );
      ...
      const webhookPayload = {
        ...
        nombre_paiements_alma:
          invoice.paymentMethod && invoice.paymentMethod.includes('ALMA')
            ? invoice.nombreChequesAVenir || 0
            : 0,
        montant_par_paiement_alma:
          invoice.paymentMethod &&
          invoice.paymentMethod.includes('ALMA') &&
          invoice.nombreChequesAVenir &&
          invoice.nombreChequesAVenir > 0 &&
          montantRestant > 0
            ? (montantRestant / invoice.nombreChequesAVenir).toFixed(2)
        ...
        // ✅ CHAMP POUR AFFICHAGE EMAIL - MODE DE PAIEMENT DÉTAILLÉ AVEC WORDING ALMA
        mode_paiement_avec_details: (() => {
          // Si pas d'acompte ET pas de chèques/paiements à venir
          if (
            (!acompteAmount || acompteAmount === 0) &&
        ...
        // NOUVEAU : Liste simple des produits pour email (format texte lisible)
        ...
        // NOUVEAU : Noms des produits comme chaîne de caractères séparés par des virgules
        noms_produits_string: invoice.products.map(p => p.name).join(', '),

        // NOUVEAU : Résumé des produits pour sujet email
        resume_produits:
          invoice.products.length === 1
            ? invoice.products[0].name
            : `${invoice.products[0].name} et ${invoice.products.length - 1} autre${invoice.products.length > 2 ? 's' : ''} produit${invoice.products.length > 2 ? 's' : ''}`,
        ...
        produits_prix_unitaires: invoice.products.map(p =>
          p.priceTTC.toFixed(2)
        ),
        produits_prix_ht_unitaires: invoice.products.map(p =>
          p.priceHT.toFixed(2)
        ),
        produits_remises: invoice.products.map(p => p.discount || 0),
        produits_types_remises: invoice.products.map(
          p => p.discountType || 'fixed'
        ),
        produits_totaux: invoice.products.map(p =>
          (p.quantity * p.priceTTC).toFixed(2)
        ),
        produits_totaux_ht: invoice.products.map(p =>
          (p.quantity * p.priceHT).toFixed(2)
        ),
        produits_statuts_livraison: invoice.products.map(p =>
          (p as ProductWithDeliveryStatus).isPickupOnSite ? 'emporte' : 'a_livrer'
        ), // Nouveau champ pour les statuts de livraison

        // Additional metadata
        nombre_produits: invoice.products.length,

        // NOUVEAUX CHAMPS STATISTIQUES LIVRAISON
        nombre_produits_a_livrer: invoice.products.filter(
          p => !(p as ProductWithDeliveryStatus).isPickupOnSite
        ).length,
        nombre_produits_emportes: invoice.products.filter(
          p => (p as ProductWithDeliveryStatus).isPickupOnSite
        ).length,
        noms_produits_a_livrer: invoice.products
          .filter(p => !(p as ProductWithDeliveryStatus).isPickupOnSite)
          .map(p => p.name)
          .join(', '),
        noms_produits_emportes: invoice.products
          .filter(p => (p as ProductWithDeliveryStatus).isPickupOnSite)
          .map(p => p.name)
          .join(', '),
        a_une_livraison: invoice.products.some(p => !(p as ProductWithDeliveryStatus).isPickupOnSite)
          ? 'Oui'
          : 'Non',
        a_des_produits_emportes: invoice.products.some(
          p => (p as ProductWithDeliveryStatus).isPickupOnSite
        )
          ? 'Oui'
          : 'Non',
        ...
        produits: invoice.products.map(product => ({
          ...
          statut_livraison: (product as ProductWithDeliveryStatus).isPickupOnSite
            ? 'emporte'
            : 'a_livrer', // Nouveau champ pour le statut de livraison
          remise: product.discount || 0,
          type_remise: product.discountType || 'fixed',
        })),

        // Garder aussi le format texte pour compatibilité
        produits_text: invoice.products
      ...
      console.log(
        '📊 Taille payload:',
        JSON.stringify(webhookPayload).length,
        'caractères'
      );
      ...
      const criticalFields = {
        ...
        signature_image: webhookPayload.signature_image
          ? 'Incluse (base64)'
          : 'Non disponible',
        ...
        montant_par_cheque:
          webhookPayload.montant_par_cheque || 'Non applicable',
        ...
        'liste_produits_email (longueur)': `${webhookPayload.liste_produits_email.length} chars`,
        noms_produits_string: `"${webhookPayload.noms_produits_string}"`,
        resume_produits: `"${webhookPayload.resume_produits}"`,
      ...
      try {
        ...
        if (
          ...
          console.warn(
            '⚠️ Erreur CORS détectée, tentative avec mode no-cors...'
          );
          ...
          try {
            ...
            return {
              success: true,
              message:
                '✅ Envoi réussi via mode no-cors (CORS configuré côté N8N requis pour les réponses)',
        ...
        const networkMessage = `❌ Erreur réseau: ${fetchError instanceof Error ? fetchError.message : String(fetchError)}`;
      ...
      const unexpectedMessage = `❌ Erreur inattendue: ${error instanceof Error ? error.message : String(error)}`;
  ...
  static async testConnection(): Promise<{
    ...
    try {
      ...
      try {
        ...
        return {
          success: false,
          message: `❌ Erreur de connexion: ${fetchError instanceof Error ? fetchError.message : String(fetchError)}`,
        };
      }
    } catch (error: unknown) {
      return {
