...
export class N8nBlueprintWebhookService {
  ...
  static async sendInvoiceToN8nBlueprint(
    ...
    try {
      ...
      const { payload, formData } = N8nBlueprintAdapter.adaptForN8nBlueprint(invoice, pdfBase64);
      ...
      console.log('🔢 Nombre de champs:', Array.from(formData.entries()).length);
      ...
      try {
        ...
        if (response.ok) {
          ...
          console.log('🎉 Votre workflow "Workflow Facture Universel" est déclenché');
        } else {
          console.error('❌ BLUEPRINT N8N A REJETÉ LA FACTURE');
          console.error('🔍 Vérifiez votre workflow N8N et les champs attendus');
        }
        console.groupEnd();
  ...
  static async testBlueprintConnection(): Promise<{
    ...
    try {
      ...
      testFormData.append('date_facture', new Date().toISOString().slice(0, 10));
