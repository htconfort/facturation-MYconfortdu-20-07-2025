...
export async function generateInvoicePDF(
  ...
  if (invoiceData.totalDiscount > 0) {
    ...
    doc.text(`-${formatEuro(invoiceData.totalDiscount)}`, 185, yPos, {
      align: 'right',
    });
    yPos += 7;
    doc.setTextColor(20, 40, 29);
  ...
  if (invoiceData.depositAmount && invoiceData.depositAmount > 0) {
    ...
    doc.text(formatEuro(invoiceData.depositAmount), 185, yPos, {
      align: 'right',
    });
    ...
    doc.text(
      formatEuro(invoiceData.totalTTC - invoiceData.depositAmount),
      185,
      yPos,
      { align: 'right' }
    );
  ...
  if (invoiceData.signature) {
    try {
      ...
      if (invoiceData.signature.startsWith('data:image')) {
        ...
        doc.setFontSize(10);
        doc.text(invoiceData.signature, 160, signatureY + 15, {
          align: 'center',
        });
      }
      ...
      doc.text(`Signé le ${signatureDate}`, 160, signatureY + 22, {
        align: 'center',
      });
    } catch (error) {
  ...
  if (invoiceData.paymentMethod) {
    ...
    doc.setFontSize(9);
    doc.text(
      `Mode de règlement : ${invoiceData.paymentMethod}`,
      15,
      notesY + 7
    );
    notesY += 20;
  ...
  doc.setFontSize(8);
  doc.setFont('helvetica', 'normal');
  doc.text('Merci de votre confiance !', 105, pageHeight - 12, {
    align: 'center',
  });
  doc.text(
...
export function convertInvoiceToCustomFormat(
  invoice: Invoice
): CustomInvoiceData {
