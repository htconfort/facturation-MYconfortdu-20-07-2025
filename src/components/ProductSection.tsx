...
import {
  proposerAcomptePourChequesRonds,
  formatMessageOptimisation,
  calculerGainTemps,
  optimisationBenefique,
} from '../utils/chequeOptimization';
...
export const ProductSection: React.FC<ProductSectionProps> = ({
  ...
  const totals = useMemo(() => {
    ...
    return {
      ...
      montantParCheque:
        totalARecevoirNum && chequesQuantityNum
          ? totalARecevoirNum / chequesQuantityNum
          : 0,
      totalCheques:
        totalARecevoirNum && chequesQuantityNum ? totalARecevoirNum : 0,
    };
  ...
  const optimisation = useMemo(() => {
    ...
    const proposition = proposerAcomptePourChequesRonds(
      totals.totalWithTax,
      nombreChequesAVenir
    );
    const estBenefique = optimisationBenefique(
      totals.totalWithTax,
      nombreChequesAVenir
    );
    ...
    return {
      ...
      peutOptimiser:
        estBenefique && proposition.acompte !== (acompteAmount || 0),
    };
  ...
  useEffect(() => {
    ...
    if (
      ...
      (nombreChequesAVenir === 0 || nombreChequesAVenir === null)
    ) {
  ...
  useEffect(() => {
    ...
    if (paymentMethod === 'Virement') {
      ...
      if (totals.totalWithTax > 0) {
        const acompteObligatoire =
          Math.round(totals.totalWithTax * 0.2 * 100) / 100; // Arrondi à 2 décimales
  ...
  useEffect(() => {
    ...
    if (paymentMethod === 'Alma') {
      ...
      if (
        nombreChequesAVenir === 0 ||
        nombreChequesAVenir === null ||
        nombreChequesAVenir > 4
      ) {
  ...
  const handleChequesQuantityChange = (
  ...
  const addProduct = () => {
    ...
    const selectedCatalogProduct = productCatalog.find(
    ...
    const product: Product = {
      ...
      autoCalculateHT: isCustomProduct
        ? false
        : selectedCatalogProduct?.autoCalculateHT || false,
    ...
    const existingIndex = products.findIndex(
      p =>
        p.name === product.name &&
        Math.abs(p.priceTTC - product.priceTTC) < 0.01
    );
  ...
  return (
    <div className='bg-[#477A0C] rounded-xl shadow-[0_10px_25px_-5px_rgba(0,0,0,0.3)] p-6 mb-6 transform transition-all hover:scale-[1.005] hover:shadow-[0_15px_30px_-5px_rgba(0,0,0,0.4)]'>
      ...
      <div className='bg-[#F2EFE2] rounded-lg p-6 mt-4'>
          ...
          {newProduct.priceTTC > 0 && (
            <div className='mt-2 text-sm text-black'>
              <span className='font-semibold'>
                Prix HT calculé: {formatCurrency(newProduct.unitPrice)}
              </span>
            </div>
          )}
        </div>
        ...
        {products.length > 0 && <DeliveryStatusSummary products={products} />}
                  ...
                  <td className='border border-gray-300 px-3 py-2 text-right'>
                    {editingIndex === index ? (
                      <input
                        ...
                        onChange={e =>
                          updateProduct(index, {
                            priceTTC: parseFloat(e.target.value) || 0,
                          })
                        }
      ...
      <div className='mt-6 grid grid-cols-1 lg:grid-cols-3 gap-6'>
        ...
        <div className='bg-[#FFE4B5] rounded-lg p-4 border-2 border-[#477A0C]'>
          ...
          <div className='bg-gradient-to-r from-purple-50 to-pink-50 border-2 border-purple-300 rounded-lg p-4'>
            <div className='flex items-center mb-3'>
              ...
              <h4 className='font-bold text-purple-800 bg-[#F55D3E] text-white px-4 py-2 rounded-lg border-2 border-[#F55D3E]'>
                TOTAL À RECEVOIR
              </h4>
            </div>
            ...
            <div className='grid grid-cols-1 gap-3'>
              ...
              <div>
                ...
                <div className='text-xs text-purple-600 mt-1'>
                  ✨ Synchronisé avec "TOTAUX & ACOMPTE" (
                  {formatCurrency(totals.totalWithTax)} -{' '}
                  {formatCurrency(Number(acompteAmount || 0))})
                </div>
              </div>
              ...
              <div>
                <label className='block text-purple-700 font-semibold mb-1 flex items-center'>
                  ...
                  {paymentMethod === 'Alma'
                    ? 'Montant par paiement (calculé)'
                    : 'Montant par chèque (calculé)'}
                </label>
                <input
                  type='text'
                  value={
                    totals.montantParCheque > 0
            ...
            {totals.totalARecevoir > 0 && Number(nombreChequesAVenir) > 0 && (
              <div className='mt-3 p-3 bg-purple-100 border border-purple-300 rounded-lg'>
                ...
                <div className='text-xs text-purple-600'>
                  ...
                  {nombreChequesAVenir}{' '}
                  {paymentMethod === 'Alma' ? 'paiement' : 'chèque'}
                  {Number(nombreChequesAVenir) > 1 ? 's' : ''} ={' '}
                  {formatCurrency(totals.montantParCheque)} par{' '}
                  {paymentMethod === 'Alma' ? 'paiement' : 'chèque'}
                </div>
                <div className='mt-2 p-2 bg-green-100 border border-green-300 rounded'>
                  <div className='text-sm text-green-800 font-semibold'>
                    ✅ Total {paymentMethod === 'Alma' ? 'ALMA' : 'des chèques'}{' '}
            ...
            */}
            ...
            <div className='mt-3 p-3 bg-green-50 border border-green-200 rounded text-xs text-green-700'>
              ✨ <strong>Synchronisation automatique :</strong> Le "Total à
              recevoir" est automatiquement calculé (Total TTC - Acompte) et
              synchronisé avec le bloc "TOTAUX & ACOMPTE". Saisissez uniquement
              le nombre{' '}
              {paymentMethod === 'Alma' ? 'de paiements ALMA' : 'de chèques'}{' '}
              pour calculer automatiquement le montant par{' '}
              {paymentMethod === 'Alma' ? 'paiement' : 'chèque'}.
            </div>
          </div>
        </div>
              ...
              <div className='space-y-3'>
                ...
                <div>
                  <label className='block text-blue-700 font-semibold mb-1 flex items-center'>
                    ...
                    {optimisation &&
                      optimisation.estBenefique &&
                      acompteAmount === optimisation.acompte && (
                    ...
                    acompteAmount === optimisation.acompte && (
                      <div className='mb-2 p-2 bg-blue-50 border border-blue-200 rounded-lg'>
                        <div className='text-xs text-blue-700 flex items-center'>
                          ...
                          <strong>Proposition automatique</strong> : Vous pouvez
                          modifier ce montant si nécessaire
                        </div>
                      </div>
                    )}
                  <input
                    ...
                    acompteAmount === optimisation.acompte && (
                      <div className='mt-2 p-3 bg-green-100 border border-green-300 rounded-lg'>
                        <div className='flex items-center text-sm text-green-800 mb-2'>
                          ...
                          <span className='font-semibold'>
                            🎯 Proposition optimale : {optimisation.nbCheques}{' '}
                            chèques de {optimisation.montantCheque}€ pile
                          </span>
                        </div>
                        {optimisation.gainTemps && (
                        ...
                        <div className='text-xs text-green-600 bg-green-50 p-2 rounded border border-green-200'>
                          ✏️ <strong>Modifiable :</strong> Vous pouvez ajuster
                          ce montant selon les besoins du client
                        </div>
                      </div>
                    )}
                </div>
                ...
                {totals.totalWithTax > 0 && (
                  <div className='mt-3'>
                    <div className='flex justify-between text-xs text-gray-600 mb-1'>
                      ...
                      <span>
                        {Math.round(
                          (Number(acompteAmount || 0) / totals.totalWithTax) *
                            100
                        )}
                        %
                      </span>
                    ...
                    <div className='w-full bg-gray-200 rounded-full h-3'>
                      <div
                        ...
                        style={{
                          width: `${Math.min((Number(acompteAmount || 0) / totals.totalWithTax) * 100, 100)}%`,
                        }}
                      ></div>
                    </div>
                  </div>
                    ...
                    <div className='flex items-center text-xs text-black'>
                      ...
                      <div>
                        ...
                        <p className='text-xs mt-1 text-black'>
                          ...
                          <br />
                          • Merci d'indiquer votre nom et numéro de facture en
                          référence
                          <br />• Le RIB sera automatiquement joint à la facture
                          par email
                        </p>
                      </div>
                    </div>
        ...
        <div className='bg-[#FFE4B5] rounded-lg p-4 border-2 border-[#477A0C]'>
          ...
          <div className='space-y-4'>
              ...
              {paymentMethod === 'Chèques à venir' && (
                <div className='mt-3 p-3 bg-purple-50 border border-purple-200 rounded-lg'>
                  ...
                  <select
                    ...
                    onChange={e =>
                      handleChequesQuantityFromSelector(Number(e.target.value))
                    }
                    className='w-full border-2 border-purple-300 rounded-lg px-3 py-2 focus:border-purple-500 focus:ring-2 focus:ring-purple-200 transition-all bg-white text-purple-800 font-bold'
                  >
                  ...
                  <div className='text-xs text-purple-600 mt-1'>
                    🎯 Le nombre de chèques sera automatiquement appliqué dans
                    "Chèques à venir"
                  </div>
                </div>
              )}
              ...
              {paymentMethod === 'Alma' && (
                <div className='mt-3 p-3 bg-orange-50 border border-orange-200 rounded-lg'>
                  ...
                  <div className='text-xs text-orange-600 mt-1'>
                    💳 Le paiement ALMA sera divisé en{' '}
                    {nombreChequesAVenir || 3} fois
                  </div>
                </div>
              )}
              {isPaymentMethodEmpty() && (
            ...
            <div
              className={`bg-gradient-to-r from-green-50 to-emerald-50 border-2 rounded-lg p-4 ${
            ...
            >
              ...
              {areTermsAccepted() ? (
                ...
                <div className='mt-2 text-red-600 text-xs font-semibold'>
                  ⚠️ L'acceptation des conditions générales de vente est
                  obligatoire
                </div>
              )}
            </div>
              ...
              <div className='text-sm font-semibold'>
                {!isPaymentMethodEmpty() && areTermsAccepted() ? (
                  <div className='text-green-800 flex items-center'>
                    <CheckCircle className='w-4 h-4 mr-2' />✅ Tous les champs
