          ...
          <div className="bg-white shadow-lg rounded-lg overflow-hidden max-w-[210mm] mx-auto">
            ...
            <div className="bg-gradient-to-r from-[#477A0C] to-[#5A8F0F] text-white p-6">
              <div className="flex justify-between items-center">
                <div>
                  ...
                  <p className="text-sm opacity-90">Facturation professionnelle avec signature électronique</p>
                </div>
                {invoice.signature && (
            ...
            <div className="p-6">
              <div className="grid grid-cols-2 gap-6 mb-6">
                {/* Infos entreprise */}
                <div>
                  <h3 className="text-lg font-bold text-[#477A0C] mb-3">MYCONFORT</h3>
                ...
                <div className="text-right">
                  <div className="space-y-2">
                    <div><strong>N° Facture:</strong> {invoice.invoiceNumber}</div>
                    <div><strong>Date:</strong> {new Date(invoice.invoiceDate).toLocaleDateString('fr-FR')}</div>
                    {invoice.eventLocation && (
                      <div><strong>Lieu:</strong> {invoice.eventLocation}</div>
                    )}
                    {invoice.advisorName && (
                      <div><strong>Conseiller:</strong> {invoice.advisorName}</div>
                    )}
                  </div>
                </div>
              </div>
              ...
              <div className="bg-gray-50 p-4 rounded mb-6">
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    <p><strong>Tél:</strong> {invoice.clientPhone}</p>
                    <p><strong>Email:</strong> {invoice.clientEmail}</p>
                    {invoice.clientDoorCode && (
                      <p><strong>Code porte:</strong> {invoice.clientDoorCode}</p>
                    )}
                  </div>
                </div>
              </div>
                ...
                <table className="w-full border-collapse border border-gray-300">
                  <thead>
                    <tr className="bg-gray-100">
                      ...
                      <th className="border border-gray-300 p-2 text-right">Prix TTC</th>
                      <th className="border border-gray-300 p-2 text-right">Remise</th>
                      <th className="border border-gray-300 p-2 text-right">Total TTC</th>
                    </tr>
                  </thead>
                  <tbody>
                    {invoice.products.map((product, index) => {
                      ...
                      return (
                        <tr key={index}>
                          ...
                          <td className="border border-gray-300 p-2 text-right">{formatCurrency(product.priceTTC)}</td>
              ...
              <div className="text-right space-y-2 mb-6">
                <div className="text-lg">
                  <strong>TOTAL HT: {formatCurrency(total / (1 + invoice.taxRate / 100))}</strong>
                </div>
                <div className="text-lg">
                  <strong>TVA ({invoice.taxRate}%): {formatCurrency(total - (total / (1 + invoice.taxRate / 100)))}</strong>
                </div>
                ...
                {invoice.montantAcompte > 0 && (
                  <div className="text-lg text-orange-600 mt-2">
                    <p>Acompte versé: {formatCurrency(invoice.montantAcompte)}</p>
                    <p className="font-bold">Reste à payer: {formatCurrency(total - invoice.montantAcompte)}</p>
                  </div>
                )}
              </div>
              ...
              {(invoice.paymentMethod || invoice.deliveryMethod) && (
                <div className="grid grid-cols-2 gap-4 mb-6">
                  {invoice.paymentMethod && (
                    <div>
                      <h4 className="font-bold text-[#477A0C] mb-2">PAIEMENT</h4>
                      <p>{invoice.paymentMethod}</p>
                    </div>
                  )}
                  {invoice.deliveryMethod && (
                    <div>
                      <h4 className="font-bold text-[#477A0C] mb-2">LIVRAISON</h4>
                      <p>{invoice.deliveryMethod}</p>
                      {invoice.deliveryNotes && <p className="text-sm text-gray-600">{invoice.deliveryNotes}</p>}
                    </div>
                  )}
                </div>
              )}
              ...
              {invoice.invoiceNotes && (
                <div className="mb-6">
                  ...
                  <p className="text-sm bg-gray-50 p-3 rounded">{invoice.invoiceNotes}</p>
                </div>
              )}
              ...
              {invoice.signature && (
                <div className="text-center mt-6 p-4 bg-green-50 rounded">
                  <p className="font-bold text-green-800">✅ Facture signée électroniquement</p>
                  {invoice.signatureDate && (
                    <p className="text-sm text-green-600">
                      Signée le {new Date(invoice.signatureDate).toLocaleDateString('fr-FR')} à {new Date(invoice.signatureDate).toLocaleTimeString('fr-FR')}
                    </p>
                  )}
                </div>
              )}
