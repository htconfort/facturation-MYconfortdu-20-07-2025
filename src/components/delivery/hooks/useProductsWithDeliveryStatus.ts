...
export const useProductsWithDeliveryStatus = (
  initialProducts: ProductWithDeliveryStatus[] = []
) => {
  const [products, setProducts] =
    useState<ProductWithDeliveryStatus[]>(initialProducts);
  ...
  useEffect(() => {
    const productsWithStatus = products.map(product => ({
      ...product,
      statut_livraison:
        product.statut_livraison || ('pending' as DeliveryStatus),
    }));
  ...
  const addProduct = (
    product: Omit<ProductWithDeliveryStatus, 'statut_livraison'>
  ) => {
