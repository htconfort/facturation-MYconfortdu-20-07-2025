...
import { DeliveryStatus, DeliveryStatusSelector, ProductWithDeliveryStatus } from './DeliveryStatusSelector';
...
export const ProductsWithStatusSection: React.FC<{
  ...
  const handleStatusChange = (productIndex: number, newStatus: DeliveryStatus) => {
    const updatedProducts = products.map((product, index) => {
      if (index === productIndex) {
        return {
          ...
          emporte: newStatus === 'delivered' ? 'Emporté' : 
                  newStatus === 'to_deliver' ? 'À livrer' :
                  newStatus === 'cancelled' ? 'Annulé' : 'En attente'
        };
      }
      return product;
    });
  ...
  const getGlobalStatus = (): DeliveryStatus => {
    ...
    const deliveredCount = products.filter(p => p.statut_livraison === 'delivered').length;
    const toDeliverCount = products.filter(p => p.statut_livraison === 'to_deliver').length;
    const cancelledCount = products.filter(p => p.statut_livraison === 'cancelled').length;
    ...
    <div className="products-with-status-section">
                ...
                <div className="product-details">
                  ...
                  {product.remise > 0 && (
                    <span className="discount">
                      Remise: -{product.remise}{product.type_remise === 'percent' ? '%' : '€'}
                    </span>
                  )}
                </div>
              </div>
      ...
      <div className="status-summary">
        <div className="summary-item">
          ...
          <span>{products.filter(p => p.statut_livraison === 'pending').length}</span>
        </div>
        <div className="summary-item">
          <StatusBadge status="delivered" size="small" />
          <span>{products.filter(p => p.statut_livraison === 'delivered').length}</span>
        </div>
        <div className="summary-item">
          <StatusBadge status="to_deliver" size="small" />
          <span>{products.filter(p => p.statut_livraison === 'to_deliver').length}</span>
        </div>
        <div className="summary-item">
          <StatusBadge status="cancelled" size="small" />
          <span>{products.filter(p => p.statut_livraison === 'cancelled').length}</span>
        </div>
      </div>
    </div>
  );
