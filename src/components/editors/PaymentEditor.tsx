    ...
    >
      <div className="space-y-6">
        ...
        <div className="bg-[#477A0C] text-white p-4 rounded-lg">
          <div className="text-lg font-bold">Total facture : {totalAmount.toFixed(2)} € TTC</div>
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {[
              { value: 'cheque', label: '📝 Chèque(s)', desc: 'Paiement par chèque(s)' },
              { value: 'cash', label: '💸 Espèces', desc: 'Paiement comptant' },
              { value: 'transfer', label: '🏪 Virement', desc: 'Virement bancaire' },
              { value: 'alma', label: '💳 Alma', desc: 'Paiement en plusieurs fois' },
            ].map(option => (
              <button
                key={option.value}
                onClick={() => setForm({ ...form, paymentMethod: option.value as any })}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <label className="flex flex-col gap-2">
            <span className="text-sm font-medium text-gray-700">Acompte à l'installation</span>
            <input 
              ...
              step="10"
              value={form.acompte}
              onChange={e => setForm({ ...form, acompte: parseFloat(e.target.value) || 0 })}
              placeholder="0"
            />
          </label>
          ...
          {form.paymentMethod === 'cheque' && (
            <label className="flex flex-col gap-2">
              <span className="text-sm font-medium text-gray-700">Nombre de chèques (max 10)</span>
              <input 
                className="border border-gray-300 rounded-lg px-4 py-3 text-base focus:ring-2 focus:ring-[#477A0C] focus:border-transparent"
                type="number"
                min="1"
                ...
                onChange={e => setForm({ ...form, nombreCheques: parseInt(e.target.value) || 1 })}
              />
            </label>
          )}
        </div>
        ...
        {form.paymentMethod === 'cheque' && form.nombreCheques > 0 && (
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
            <h3 className="font-medium text-blue-800 mb-3">✨ Suggestions d'acomptes "magiques" (chèques ronds)</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
              {generateSuggestions().slice(0, 4).map((suggestion, index) => (
                <button
                  key={index}
                  ...
                  className={`p-3 bg-white rounded border text-left hover:border-blue-300 transition-colors ${
                    suggestion.quality === 'excellent' ? 'ring-1 ring-green-300' : ''
                  }`}
                >
        ...
        {form.acompte > 0 && (
          <div className="bg-gray-50 p-4 rounded-lg">
            <h3 className="font-medium text-gray-800 mb-2">📊 Récapitulatif paiement</h3>
            <div className="space-y-1 text-sm">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-medium">{calculateResteAPayer().toFixed(2)} €</span>
              </div>
              {form.paymentMethod === 'cheque' && form.nombreCheques > 0 && (
                <div className="flex justify-between border-t pt-1">
                  <span>{form.nombreCheques} chèques de :</span>
                  <span className="font-medium">{calculateMontantParCheque().toFixed(2)} € chacun</span>
                </div>
              )}
            </div>
          </div>
        ...
        {/* Notes paiement */}
        <label className="flex flex-col gap-2">
          <span className="text-sm font-medium text-gray-700">Notes sur le paiement</span>
          <textarea 
        ...
        <div className="text-xs text-gray-500 p-3 bg-gray-50 rounded-lg">
          <span className="font-medium">💡 Aide iPad :</span> Utilisez les suggestions d'acomptes magiques 
          pour obtenir des chèques avec des montants ronds (sans centimes). Maximum 10 chèques autorisés.
        </div>
      </div>
    </WizardSheet>
  );
