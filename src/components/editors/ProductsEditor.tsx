    ...
    >
      <div className="space-y-6">
        ...
        {products.map((product, index) => (
          <div key={index} className="bg-gray-50 rounded-lg p-4 space-y-4">
            <div className="flex items-center justify-between">
              <span className="font-medium text-gray-700">Produit {index + 1}</span>
              {products.length > 1 && (
                <button
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <label className="flex flex-col gap-2">
                <span className="text-sm font-medium text-gray-700">Nom du produit *</span>
                <input 
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <label className="flex flex-col gap-2">
                <span className="text-sm font-medium text-gray-700">Prix unitaire HT *</span>
                <input 
                  className="border border-gray-300 rounded-lg px-4 py-3 text-base focus:ring-2 focus:ring-[#477A0C] focus:border-transparent"
                  type="number"
                  min="0"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <label className="flex flex-col gap-2">
                <span className="text-sm font-medium text-gray-700">Type de remise</span>
                <select 
                  className="border border-gray-300 rounded-lg px-4 py-3 text-base focus:ring-2 focus:ring-[#477A0C] focus:border-transparent"
                  value={product.discountType}
                  onChange={e => updateProduct(index, { discountType: e.target.value as 'percentage' | 'fixed' })}
                >
            ...
            {product.discount > 0 && (
              <div className="text-sm text-gray-600 bg-white p-3 rounded border">
                <div>Sous-total : {(product.price * product.quantity).toFixed(2)} €</div>
                <div>Remise : -{product.discountType === 'percentage' 
        ...
        <div className="bg-[#477A0C] text-white p-4 rounded-lg">
          <div className="text-lg font-bold">Total HT : {calculateTotal().toFixed(2)} €</div>
          <div className="text-sm opacity-90">TVA 20% : {(calculateTotal() * 0.2).toFixed(2)} €</div>
          <div className="text-xl font-bold">Total TTC : {(calculateTotal() * 1.2).toFixed(2)} €</div>
        </div>

        {/* Aide iPad */}
        <div className="text-xs text-gray-500 p-3 bg-gray-50 rounded-lg">
          <span className="font-medium">💡 Astuce iPad :</span> Touchez les champs pour les modifier. 
          Utilisez le bouton + pour ajouter des produits et la corbeille pour les supprimer.
        </div>
      </div>
    </WizardSheet>
  );
