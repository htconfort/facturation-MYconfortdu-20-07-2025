    ...
    <div style={{ padding: '20px', fontFamily: 'Arial, sans-serif' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px', marginBottom: '20px' }}>
        ...
        <div style={{ border: '2px solid #3B82F6', borderRadius: '8px', padding: '16px' }}>
              ...
              {testInvoice.products.map((product, index) => {
                ...
                return (
                  <tr key={index}>
                    ...
                    <td style={{ padding: '8px', border: '1px solid #ddd', textAlign: 'right' }}>{formatCurrency(product.priceTTC)}</td>
                    <td style={{ padding: '8px', border: '1px solid #ddd', textAlign: 'right' }}>
                      {product.discount > 0 ? `${product.discount}${product.discountType === 'percent' ? '%' : '€'}` : '-'}
                    </td>
                    <td style={{ padding: '8px', border: '1px solid #ddd', textAlign: 'right', fontWeight: 'bold' }}>{formatCurrency(productTotal)}</td>
                  </tr>
                );
              })}
            </tbody>
          ...
          <div style={{ marginTop: '16px', padding: '12px', background: '#EBF8FF', borderRadius: '4px' }}>
            <strong>Total calculé dans l'aperçu :</strong><br/>
            {formatCurrency(testInvoice.products.reduce((sum, product) => {
        ...
        <div style={{ border: '2px solid #EF4444', borderRadius: '8px', padding: '16px' }}>
          ...
          <p style={{ color: '#6B7280', fontSize: '14px' }}>
            Le PDF utilise convertInvoiceData() qui mappe invoice.products vers data.items.
            <br/>Cliquez sur "Tester PDF" et regardez les logs de la console pour voir ce qui se passe.
          </p>
          ...
          <div style={{ marginTop: '16px', padding: '12px', background: '#FEF2F2', borderRadius: '4px' }}>
            <strong>Instructions :</strong><br/>
            1. Cliquez sur "Tester PDF"<br/>
            2. Regardez les logs de la console<br/>
            3. Comparez les montants dans le PDF généré<br/>
            4. Si les montants diffèrent, le problème est dans convertInvoiceData()
          </div>
        </div>
      </div>
